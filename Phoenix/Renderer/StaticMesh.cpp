...
StaticMesh::StaticMesh(RenderDevice* device, MemoryHeap* memoryHeap, char* vertexData, uint32_t vertexDataSize, char* indexData,
    ...
    : mDevice(device), mIndexCount(indexCount)
{
	mVertexBuffer = std::unique_ptr<Buffer>(
	    new Buffer(mDevice, memoryHeap, vertexDataSize,
	...
	mIndexBuffer = std::unique_ptr<Buffer>(
	    new Buffer(mDevice, memoryHeap, indexDataSize,
	               VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
	               VK_SHARING_MODE_EXCLUSIVE));
...
void StaticMesh::Use(VkCommandBuffer* commandBuffer, uint32_t index)
{
	...
	vkCmdBindIndexBuffer(commandBuffer[index], mIndexBuffer->GetBuffer(), 0, VK_INDEX_TYPE_UINT32);
}

void StaticMesh::Draw(VkCommandBuffer* commandBuffer, uint32_t index) { vkCmdDrawIndexed(commandBuffer[index], mIndexCount, 1, 0, 0, 0); }
