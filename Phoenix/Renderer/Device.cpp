    ...
    : mWindowWidth(windowWidth), mWindowHeight(windowHeight)
{
	...
	vkEnumerateInstanceExtensionProperties(nullptr, &supportedPropertiesCount, nullptr);
	std::vector<VkExtensionProperties> supportedProperties;
	supportedProperties.resize(supportedPropertiesCount);
	vkEnumerateInstanceExtensionProperties(nullptr, &supportedPropertiesCount, supportedProperties.data());
	...
	Validate(vkCreateInstance(&instanceCreateInfo, NULL, &mInstance));
	...
	if (Validation)
	{
		PFN_vkCreateDebugReportCallbackEXT CreateDebugReportCallbackEXT =
		    reinterpret_cast<PFN_vkCreateDebugReportCallbackEXT>(vkGetInstanceProcAddr(mInstance, "vkCreateDebugReportCallbackEXT"));
		...
		Validate(CreateDebugReportCallbackEXT(mInstance, &debugReportCallbackInfo, nullptr, &mDebugReportCallback));
	}
	...
	const char*    requiredDeviceExtensions[requiredDeviceExtentionCount] = {VK_KHR_SWAPCHAIN_EXTENSION_NAME,
			...
			if (GetQueueFamily(physicalDevices[deviceIndex], VkQueueFlagBits::VK_QUEUE_GRAPHICS_BIT, queue_family))
			{
				...
				if (mPhysicalDevice == VK_NULL_HANDLE ||
				    mPhysicalDevice != VK_NULL_HANDLE &&
				        physicalDeviceProperties.deviceType == VkPhysicalDeviceType::VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
				{
	...
	Validate(vkCreateDevice(mPhysicalDevice, &deviceCreateInfo, nullptr, &mDevice));

	// volkLoadDevice( mDevice );

	vkGetDeviceQueue(mDevice, mPhysicalDevicesQueueFamily, 0, &mGraphicsQueue);
	...
	Validate(vkCreateCommandPool(mDevice, &pool_info, nullptr, &mCommandPool));
	...
	mDepthImageFormat = FindSupportedFormat(candidateDepthFormats, candidateDepthFormatCount, VK_IMAGE_TILING_OPTIMAL,
	...
	for (uint32_t i = 0; i < mSwapchainImageCount; i++)
	{
		...
		Validate(vkCreateFence(mDevice, &fenceCreateinfo, nullptr, &mSwapchainImageFence.get()[i]));
	}

	VkSemaphoreCreateInfo semaphoreCreateInfo = {};
	semaphoreCreateInfo.sType                 = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;

	Validate(vkCreateSemaphore(mDevice, &semaphoreCreateInfo, nullptr, &mImageAvailableSemaphore));

	Validate(vkCreateSemaphore(mDevice, &semaphoreCreateInfo, nullptr, &mRenderFinishedSemaphore));
	...
	Validate(vkAllocateCommandBuffers(mDevice, &commandBufferAllocateInfo, mPrimaryCommandBuffers.get()));
...
RenderDevice::~RenderDevice()
{
	...
	{
		vkDestroyFence(mDevice, mSwapchainImageFence.get()[i], nullptr);
	}
                               ...
                               VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& image_memory, VkImageLayout initialLayout)
{
	...
	Validate(vkCreateImage(mDevice, &imageCreateInfo, nullptr, &image));

	VkMemoryRequirements memRequirments;
	vkGetImageMemoryRequirements(mDevice, image, &memRequirments);

	uint32_t memoryType = FindMemoryType(memRequirments.memoryTypeBits, properties);
	...
	Validate(vkAllocateMemory(mDevice, &memoryAllocationInfo, nullptr, &image_memory));

	Validate(vkBindImageMemory(mDevice, image, image_memory, 0));
}
...
void RenderDevice::EndCommand(VkCommandBuffer& commandBuffer)
{
	...
	Validate(vkQueueSubmit(mGraphicsQueue, 1, &submitInfo, VK_NULL_HANDLE));
...
VkCommandBuffer RenderDevice::CreateSingleTimeCommand()
{
	...
	Validate(vkAllocateCommandBuffers(mDevice, &commandBufferAllocationInfo, &commandBuffer));
...
VkShaderModule RenderDevice::CreateShaderModule(char* data, uint32_t size)
{
	...
	Validate(vkCreateShaderModule(mDevice, &shaderModuleCreateInfo, nullptr, &shaderModule));

	return shaderModule;
}

ResourceTableLayout* RenderDevice::GetPostProcessSampler() { return mSamplerResourceTableLayout.get(); }
...
{
	vkWaitForFences(mDevice, 1, &mSwapchainImageFence.get()[mSwapchainImageIndex], VK_TRUE, UINT32_MAX);

	Validate(vkAcquireNextImageKHR(mDevice, mSwapChain, UINT64_MAX, mImageAvailableSemaphore, VK_NULL_HANDLE, &mSwapchainImageIndex));

	Validate(vkQueueWaitIdle(mGraphicsQueue));

	Validate(vkResetFences(mDevice, 1, &mSwapchainImageFence.get()[mSwapchainImageIndex]));

	mRenderSubmitInfo.pCommandBuffers = &mPrimaryCommandBuffers.get()[mSwapchainImageIndex];

	Validate(vkQueueSubmit(mGraphicsQueue, 1, &mRenderSubmitInfo, mSwapchainImageFence.get()[mSwapchainImageIndex]));

	mPresentInfo.pImageIndices = &mSwapchainImageIndex;

	Validate(vkQueuePresentKHR(mGraphicsQueue, &mPresentInfo));
...
void RenderDevice::CreateSwapchain()
{
	...
	swapchainCreateInfo.imageUsage =
	    VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // | VK_IMAGE_USAGE_STORAGE_BIT; // VK_IMAGE_USAGE_STORAGE_BIT used for raytracing
	...
	Validate(vkCreateSwapchainKHR(mDevice, &swapchainCreateInfo, nullptr, &mSwapChain));

	Validate(vkGetSwapchainImagesKHR(mDevice, mSwapChain, &mSwapchainImageCount, nullptr));

	mSwapchainImages = std::unique_ptr<VkImage>(new VkImage[mSwapchainImageCount]);

	Validate(vkGetSwapchainImagesKHR(mDevice, mSwapChain, &mSwapchainImageCount, mSwapchainImages.get()));

	mSwapchainImageViews = std::unique_ptr<VkImageView>(new VkImageView[mSwapchainImageCount]);

	for (uint32_t i = 0; i < mSwapchainImageCount; i++)
	{
		TransitionImageLayout(mSwapchainImages.get()[i], mSurfaceFormat.format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
		...
		Validate(vkCreateImageView(mDevice, &imageViewCreateInfo, nullptr, &mSwapchainImageViews.get()[i]));
	}
}
...
void RenderDevice::DestroySwapchain()
{
	for (uint32_t i = 0; i < mSwapchainImageCount; i++)
	{
		vkDestroyImageView(mDevice, mSwapchainImageViews.get()[i], nullptr);
	}
	vkDestroySwapchainKHR(mDevice, mSwapChain, nullptr);
}

bool RenderDevice::HasRequiredExtentions(const VkPhysicalDevice& physicalDevice, const char** requiredExtentions,
...
bool RenderDevice::CheckSwapchainSupport(VkSurfaceCapabilitiesKHR& capabilities, std::unique_ptr<VkSurfaceFormatKHR[]>& formats,
                                         uint32_t& formatCount, std::unique_ptr<VkPresentModeKHR[]>& modes, uint32_t& modeCount)
{
	Validate(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(mPhysicalDevice, mSurface, &capabilities));
...
VkImageMemoryBarrier RenderDevice::ImageMemoryBarrier(VkImage& image, VkFormat& format, VkImageLayout& old_layout,
