...
Texture::Texture(RenderDevice* device, MemoryHeap* memoryHeap, uint32_t width, uint32_t height, VkFormat format,
    ...
    : mDevice(device), mWidth(width), mHeight(height), mImageUsageFlags(imageUsageFlags), mFormat(format), mMemoryHeap(memoryHeap),
      mOwnMemory(false)
{
...
void Texture::CreateImage(VkImageUsageFlags imageUsageFlags, char* data)
{
	...
	{
		transferBuffer =
		    std::unique_ptr<Buffer>(new Buffer(mDevice, texture_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE));
	...
	mDevice->Validate(vkCreateImage(mDevice->GetDevice(), &image_create_info, nullptr, &mImage));

	VkMemoryRequirements imageMemoryRequirments;
	vkGetImageMemoryRequirements(mDevice->GetDevice(), mImage, &imageMemoryRequirments);
	...
	mDevice->Validate(vkBindImageMemory(mDevice->GetDevice(), mImage, mMemoryHeap->GetMemory()->GetMemory(), memoryOffset));
	...
	if (data != nullptr)
	{
		...
		SetImageLayout(copy_cmd, mImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
		vkCmdCopyBufferToImage(copy_cmd, transferBuffer->GetBuffer(), mImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
		SetImageLayout(copy_cmd, mImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	...
	else
	{
		...
		SetImageLayout(copy_cmd, mImage, VK_IMAGE_LAYOUT_UNDEFINED,
		               (mImageUsageFlags & VK_IMAGE_USAGE_SAMPLED_BIT) ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL,
		               subresourceRange);
	}
