	...
	mDevice( device ), mUseDepth( useDepth )
{
	...
	mRenderpass = std::unique_ptr<RenderPass>( new RenderPass(
		mDevice,
		width, height,
		mFramebufferAttachment.get( )
	) );
}
...
{
	mImage = std::unique_ptr<Texture>( new Texture(
		mDevice,
		width,
		height,
		mFormat,
		VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
		nullptr
	) );


	mFramebufferPacket = std::unique_ptr<FramebufferPacket>( new FramebufferPacket(
		mImage.get( ),
		1,
		mFormat,
	...
	if ( mUseDepth )
	{
		mDepthImage = std::unique_ptr<Texture>( new Texture(
			mDevice,
			width,
			height,
		...
		mDepthFramebufferPacket = std::unique_ptr<FramebufferPacket>( new FramebufferPacket(
			...
			EFramebufferImageType::Depth
		) );

		mFramebufferAttachment = std::unique_ptr<FramebufferAttachment>( new FramebufferAttachment(
			mDevice,
			{
				mFramebufferPacket.get( ),
				mDepthFramebufferPacket.get()
			}
		) );
	}
	else
